...
/**
 * Used by various other classes that need to keep a list of courses. It extends
 * the List class which provides the basic functionality for storage and
...
public class CourseList extends List {
	 ...
	 *         criteria, null otherwise.
	 */
	public Course findCourseByIDActivityAndGroup(String courseID, String group,
			String type) {
		...
		while (!done) {
			...
			if (currentObject == null) {
				...
				if ((currentObject.getCourseID().compareToIgnoreCase(courseID) == 0)
						...
						&& (currentObject.getType().compareToIgnoreCase(type) == 0)) {
