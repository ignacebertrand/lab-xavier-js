...
/**
 * This class reads from the InputFile and instantiates the Teacher objects in
 * the system. It is assumed that the InputFile is in the local directory,
 * contains the active teachers, and each line of input is read and expected in
 * the following format: a field oriented and space-separated text file that
 * lists all the teachers. The fields are as follows:
 * <pre>
 *     ETS001 Champagne Roger PRF LOG120 MAT115 CHM101
 *     |      |         |     |   |
 *     |      |         |     |   Past courses taught in the same year (before this term)
 *     |      |         |     Typwe of teacher (PRF=prof, CHR = Charge de cours, e.g. T.A.)
